...
def generateUserProfiles(R, d, seed, sparkContext, N):
    ...
    numUsers = U.count()
    # generate the random rdd comprised vector of numRows=numUsers and numCols=d
    randRDD = RandomRDDs.normalVectorRDD(sparkContext, numUsers, d, numPartitions=N, seed=seed)
    # Make tuple where 1st element is value and second element is a index of value
    # rdd = [(1,2),(2,4)] would become [((1,2),0),((2,4),1)]
...
def generateItemProfiles(R, d, seed, sparkContext, N):
    ...
    """
    # exctract items
    items = R.map(lambda inp: inp[1]).distinct(numPartitions=N)
    numItems = items.count()
    randRDD = RandomRDDs.normalVectorRDD(sparkContext, numItems, d, numPartitions=N, seed=seed)
    itemsRDD = items.zipWithIndex().map(swap)
    randRDD = randRDD.zipWithIndex().map(swap)
    return itemsRDD.join(randRDD, numPartitions=N).values()
...
def joinAndPredictAll(R, U, V, N):
    ...
    """
    iRDD = R.map(lambda tu: (tu[0],(tu[1],tu[2])))
    gatheredAlli = iRDD.join(U, numPartitions=N).map(lambda tu: (tu[1][0][0], (tu[0],tu[1][0][1],tu[1][1]))) #lambda (i,((j,r),u)): (j, (i,r,u)))
    return gatheredAlli.join(V, numPartitions=N).map(lambda tu: (tu[1][0][0], tu[0], tu[1][0][1], tu[1][0][2], tu[1][1])).\
                map(lambda tu:(tu[0],tu[1],pred_diff(tu[2],tu[3],tu[4]),tu[3],tu[4])) # 1. lambda (j, ( (i, r, u), v)): (i, j, r, u, v)
                                                                                    # 2. lambda (i, j, r, u, v): (i, j, pred_diff(r, u, v), u, v)
...
def SE(joinedRDD):
    ...
    """
    SE = joinedRDD.map(lambda tu: tu[2]*tu[2]).reduce(add) #lambda (i,j,d,ui,vj): d*d
    return SE
...
def normSqRDD(profileRDD, param):
	...
	The return value is:
        λ Σ_i ||ui||_2^2
    """
    return param * profileRDD.map(lambda tu: np.dot(tu[1],tu[1])).reduce(add)
def gradient_sum(pair1,pair2):
    """ function that adds two gradients.
    """
    prof1,grad1 =pair1
    prof2,grad2 =pair2
    return (prof1, grad1+grad2)
...
def adaptU(joinedRDD, gamma, lam, N):
    ...
    """
    return joinedRDD.map(lambda tu: (tu[0], (tu[3], gradient_u(tu[2], tu[3], tu[4])))) \
        .reduceByKey(gradient_sum,N) \
        .mapValues(lambda tu: tu[0] - gamma * (tu[1] + 2 * lam * tu[0]))
...
def adaptV(joinedRDD, gamma, mu, N):
    ...
    """
    return joinedRDD.map(lambda tu: (tu[1], (tu[4], gradient_v(tu[2], tu[3], tu[4])))) \
        .reduceByKey(gradient_sum, numPartitions=N) \
        .mapValues(lambda tu: tu[0] - gamma * (tu[1] + 2 * mu * tu[0]))
